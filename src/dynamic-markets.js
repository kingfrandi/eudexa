...
function cryptoRows(assets) {
  return assets.map((asset, index) => {
    ...
    return `<div class="marketRow" data-market-asset="${esc(asset.name)}" data-market-crypto-id="${esc(asset.id || '')}">
      <div><span>Crypto</span><strong>${esc(asset.name || '—')}</strong><small>${esc(symbol)}</small></div>
...
async function loadDynamicCrypto() {
  ...
  try {
    ...
    if (note) note.textContent = `Mostrando ${assets.length.toLocaleString('en-US')} criptomonedas obtenidas automáticamente desde el catálogo de mercado.`;
  } catch (error) {
    table.innerHTML = '<div class="tableHead"><span>Activo</span><span>Gráfico</span><span>Precio</span><span>Cambio 24h</span></div><div class="dynamicLoading">No se pudo cargar el catálogo dinámico de criptomonedas.</div>';
  }
}

function scheduleDynamicCrypto() { window.setTimeout(loadDynamicCrypto, 80); }
